...
func (p *Plan) GetAll() ([]*Plan, error) {
	...
	query := `select id, plan_name, plan_amount, created_at, updated_at from plans order by plan_name`
	...
	for rows.Next() {
		var plan Plan
		err := rows.Scan(&plan.ID, &plan.PlanName, &plan.PlanAmount, &plan.CreatedAt, &plan.UpdatedAt)
		if err != nil {
